...
import { MessageSquare, Send, Globe, Lock, Paperclip, X } from "lucide-react";
...
export function RequestComments({ requestId }: RequestCommentsProps) {
  const { comments, isLoading, refetch } = useRequestComments(requestId);
  const { mutate: addComment, isPending } = useAddRequestComment();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    addComment(
      { requestId, data: { body: commentBody } },
      {
        onSuccess: async () => {
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <Textarea
            placeholder="Add a comment... (shared with request and case)"
            value={commentBody}
