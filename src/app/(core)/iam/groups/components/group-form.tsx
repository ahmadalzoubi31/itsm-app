...
import { useUsersHook } from "../../users/hooks/useUsers.hook";
import { User } from "../../users/interfaces/user.interface";
import { Search } from "lucide-react";

type GroupFormValues = z.infer<typeof groupSchema>;

const GroupForm = ({ id }: { id: string }) => {
  ...
  const onSubmit = async (data: GroupFormValues) => {
    const promise = async () => {
      ...
      if (id) {
        const { businessLineId, ...groupData } = data;
      ...
      if (groupId) {
        ...
        if (toAdd.length > 0) {
          await addUsersToGroup(groupId, toAdd);
        }
        if (toRemove.length > 0) {
          await removeUsersFromGroup(groupId, toRemove);
        }
      }
    };
          ...
          <TabsContent value="basic-info" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Enter the basic information for the new group
                </CardDescription>
              </CardHeader>
                    ...
                    {filteredUsers.length === 0 ? (
                      ...
                      <>
                        ...
                        {filteredUsers.map((user) => (
                          <div
                            key={user.id}
                            className="flex items-center space-x-2 p-3 rounded-lg hover:bg-accent transition-colors border border-transparent hover:border-border"
                          >
                            <Checkbox
                              id={`user-${user.id}`}
