...
import { Mail, Settings, Bell, TestTube, BarChart3, Download, Send } from "lucide-react";
...
import { useGetEmailSettings, useEmailStatistics } from "./hooks/useEmailSettings";
...
export default function EmailSettingsPage() {
  ...
  const { data: notificationSettings, isLoading: notificationLoading } = useGetNotificationSettings();
  const { statistics } = useEmailStatistics();

  const isOutgoingConfigured = settings?.outgoing?.enabled && settings.outgoing.host;
  const isIncomingConfigured = settings?.incoming?.enabled && settings.incoming.host;
    ...
    <div className="px-4 lg:px-6">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Configure your email engines, notifications, and test your email integration
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Outgoing Engine</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Incoming Engine</p>
                <p className="text-2xl font-bold">
      ...
      {(!isOutgoingConfigured && !isIncomingConfigured) && (
        <Alert className="mb-8 border-orange-200 bg-orange-50">
          ...
          <AlertDescription className="text-orange-800">
            Email engines are not configured. Please configure your outgoing and/or incoming email engines 
            to enable email notifications and functionality.
          </AlertDescription>
        </Alert>
      )}
          ...
          {(isOutgoingConfigured || isIncomingConfigured) ? (
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-lg font-medium mb-2">Email Engine Required</h3>
                <p className="text-gray-600 mb-4">
                  Please configure your email engines first before testing email functionality.
                </p>
                <button
...
function EmailStatisticsCard({ statistics, loading }: EmailStatisticsCardProps) {
  if (loading) {
    return (
      <Card>
        <CardContent className="p-8 text-center">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-4 border rounded-lg">
              <div className="text-2xl font-bold text-green-600">{statistics.totalSent}</div>
              <div className="text-sm text-gray-600">Total Sent</div>
            </div>
            
            <div className="p-4 border rounded-lg">
              <div className="text-2xl font-bold text-red-600">{statistics.totalFailed}</div>
              <div className="text-sm text-gray-600">Total Failed</div>
            </div>
            
            <div className="p-4 border rounded-lg">
              <div className="text-2xl font-bold text-blue-600">{statistics.deliveryRate?.toFixed(1)}%</div>
              <div className="text-sm text-gray-600">Delivery Rate</div>
            </div>
            
            <div className="p-4 border rounded-lg">
              <div className="text-2xl font-bold text-purple-600">{statistics.averageDeliveryTime}ms</div>
