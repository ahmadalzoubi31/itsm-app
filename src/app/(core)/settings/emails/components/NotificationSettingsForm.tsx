...
import { notificationSettingsSchema, NotificationSettingsForm as NotificationForm } from "../validations/email.schema";
import { useGetNotificationSettings, useSaveNotificationSettings } from "../hooks/useNotificationSettings";
import { NOTIFICATION_TYPES, NotificationTypeEnum } from "../constants/notification-type.constant";
...
export function NotificationSettingsForm({ onSuccess }: NotificationSettingsFormProps) {
  const { data: notifications, isLoading: loading, isError: error } = useGetNotificationSettings();
  const { mutate: saveNotifcationSettings, isPending: saving } = useSaveNotificationSettings();
  ...
  const handleNotificationToggle = (notificationType: NotificationTypeEnum, checked: boolean) => {
    const currentTypes = form.getValues("notificationTypes");
    if (checked) {
      form.setValue("notificationTypes", [...currentTypes, notificationType]);
    } else {
      form.setValue("notificationTypes", currentTypes.filter(type => type !== notificationType));
    }
  };

  const addRecipient = (field: "defaultRecipients" | "urgentRecipients" | "ccRecipients" | "bccRecipients", email: string) => {
  ...
  const removeRecipient = (field: "defaultRecipients" | "urgentRecipients" | "ccRecipients" | "bccRecipients", email: string) => {
    const current = form.getValues(field);
    form.setValue(field, current.filter(recipient => recipient !== email));
  };
              ...
              <>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {filteredNotifications.map((notification) => {
                      const isSelected = watchedNotificationTypes.includes(notification.value);
                      const priorityColor = PRIORITY_LEVELS.find(p => p.value === notification.priority)?.color || "gray";
                            ...
                            <div className="flex items-start gap-3">
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => 
                                  handleNotificationToggle(notification.value, checked as boolean)
                                }
                              />
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <FormDescription>
                      Email addresses that will receive all enabled notifications
                    </FormDescription>
                    <RecipientManager
                      recipients={form.watch("defaultRecipients")}
                      onAdd={(email) => addRecipient("defaultRecipients", email)}
                      onRemove={(email) => removeRecipient("defaultRecipients", email)}
                  ...
                  <div className="space-y-3">
                    ...
                    <FormDescription>
                      Additional recipients for high priority and critical notifications
                    </FormDescription>
                    <RecipientManager
                      recipients={form.watch("urgentRecipients")}
                      onAdd={(email) => addRecipient("urgentRecipients", email)}
                      onRemove={(email) => removeRecipient("urgentRecipients", email)}
                  ...
                  <div className="space-y-3">
                    ...
                    <RecipientManager
                      ...
                      onRemove={(email) => removeRecipient("ccRecipients", email)}
                    ...
                    render={({ field }) => (
                      <FormItem className="flex items-center justify-between rounded-lg border p-4">
                        <div className="space-y-0.5">
                          ...
                          <FormDescription>
                            Attach relevant files to notification emails when applicable
                          </FormDescription>
                        </div>
                        <FormControl>
                          <Switch checked={field.value} onCheckedChange={field.onChange} />
...
function RecipientManager({ recipients, onAdd, onRemove, placeholder }: RecipientManagerProps) {
