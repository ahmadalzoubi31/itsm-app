...
import { Loader2, TestTube, CheckCircle, XCircle, AlertCircle, Mail } from "lucide-react";

import { testEmailSchema, TestEmailForm as TestEmailFormType } from "../validations/email.schema";
...
export function EmailTestForm({ onTestResult }: EmailTestFormProps) {
  ...
  const [testMode, setTestMode] = useState<"connection" | "email">("connection");
            ...
            {result.details && (
              <div className="text-sm space-y-1">
                {result.details.messageId && (
                  <div>Message ID: <code className="bg-white px-1 rounded">{result.details.messageId}</code></div>
                )}
                {result.details.duration && (
                  <div>Duration: {result.details.duration}ms</div>
                )}
                {result.details.response && (
                  <div>Response: <code className="bg-white px-1 rounded">{result.details.response}</code></div>
                )}
        ...
        {testMode === "connection" && (
          <div className="space-y-4">
            <div className="p-4 border rounded-lg bg-blue-50 border-blue-200">
              <h4 className="font-medium text-blue-900 mb-2">Connection Test</h4>
              <p className="text-sm text-blue-800 mb-4">
                This test verifies that the system can connect to your email server using the configured settings. 
                It checks authentication, server connectivity, and basic configuration without sending any emails.
              </p>
              <Button 
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <div className="p-4 border rounded-lg bg-orange-50 border-orange-200">
                <h4 className="font-medium text-orange-900 mb-2">Send Test Email</h4>
                <p className="text-sm text-orange-800 mb-4">
                  This test sends an actual email to verify end-to-end functionality. 
                  Make sure your email provider settings are configured and the connection test passed.
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="p-3 border rounded-lg bg-gray-50">
              ...
              <p className="text-xs text-gray-600">
                Tests server connectivity and authentication without sending emails
              </p>
            </div>
            <div className="p-3 border rounded-lg bg-gray-50">
