...
import { Loader2, TestTube, Eye, EyeOff, Send, Download, Settings2, ChevronDown, ChevronUp, Shield, ShieldCheck, Lock } from "lucide-react";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "@/components/ui/collapsible";
import { toast } from "sonner";

import { EmailSettings, EmailProtocolEnum, EmailSecurityEnum } from "../types";
import { emailSettingsSchema, EmailSettingsForm } from "../validations/email.schema";
...
const OUTGOING_PROTOCOL_OPTIONS = [
  { value: EmailProtocolEnum.SMTP, label: "SMTP", description: "Simple Mail Transfer Protocol" },  
];

const INCOMING_PROTOCOL_OPTIONS = [
  { value: EmailProtocolEnum.IMAP, label: "IMAP", description: "Internet Message Access Protocol" },
  { value: EmailProtocolEnum.POP3, label: "POP3", description: "Post Office Protocol 3" },
];
...
export function EmailProviderForm({ onSuccess }: EmailProviderFormProps) {
  // Data hooks
  const { data: outgoingSettings, isLoading: outgoingLoading } = useGetOutgoingSettings();
  const { data: incomingSettings, isLoading: incomingLoading } = useGetIncomingSettings();
  ...
  const handleProtocolChange = (protocol: EmailProtocolEnum, type: 'outgoing' | 'incoming') => {
  ...
  const handleSecurityChange = (security: EmailSecurityEnum, type: 'outgoing' | 'incoming') => {
  ...
  const validatePortSecurityCombination = (protocol: EmailProtocolEnum, security: EmailSecurityEnum, port: number) => {
    ...
    if (protocol === EmailProtocolEnum.SMTP) {
      if (security === EmailSecurityEnum.SSL_TLS && port !== 465) {
        commonIssues.push(`SSL/TLS typically uses port 465, but you have ${port}`);
      }
      if (security === EmailSecurityEnum.STARTTLS && port !== 587) {
        commonIssues.push(`STARTTLS typically uses port 587, but you have ${port}`);
      }
      if (security === EmailSecurityEnum.NONE && port !== 25 && port !== 587) {
        commonIssues.push(`Unencrypted SMTP typically uses port 25 or 587, but you have ${port}`);
      }
    }
    
    if (protocol === EmailProtocolEnum.IMAP) {
      if (security === EmailSecurityEnum.SSL_TLS && port !== 993) {
        commonIssues.push(`IMAP SSL/TLS typically uses port 993, but you have ${port}`);
      }
      if (security === EmailSecurityEnum.STARTTLS && port !== 143) {
        commonIssues.push(`IMAP STARTTLS typically uses port 143, but you have ${port}`);
      }
    }
    ...
    return {
      ...
      expectedPort,
      commonIssues,
      recommendation: !isStandardPort ? `Consider using port ${expectedPort} for ${security} with ${protocol}` : null
    };
  };

  const transformDataForBackend = (data: EmailSettingsForm): BackendEmailPayload => {
    ...
    const transformedData: BackendEmailPayload = {
      ...
      outgoing: {
        ...data.outgoing,
        secure: data.outgoing.secure === EmailSecurityEnum.SSL_TLS || data.outgoing.secure === EmailSecurityEnum.STARTTLS,
      ...
      incoming: {
        ...data.incoming,
        secure: data.incoming.secure === EmailSecurityEnum.SSL_TLS || data.incoming.secure === EmailSecurityEnum.STARTTLS,
  ...
  const onSubmit = async (data: EmailSettingsForm) => {
    ...
    console.log("🚀 ~ onSubmit ~ form.formState.errors:", form.formState.errors);
    console.log("🚀 ~ onSubmit ~ form.formState.isValid:", form.formState.isValid);
    ...
    try {
      ...
      console.log("🚀 ~ onSubmit ~ Data being sent to backend:", JSON.stringify(data, null, 2));
      ...
      const savedSettings = await saveEmailMutation.mutateAsync(transformedData as unknown as Partial<EmailSettings>);
  ...
  const handleTestConnection = async () => {
    ...
    if (!outgoingEnabled && !incomingEnabled) {
      toast.error("Please enable at least one email engine (Outgoing or Incoming) to test the connection");
      return;
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Configure your outgoing and incoming email engines for notifications and ticket processing
        </CardDescription>
      </CardHeader>
        ...
        <Form {...form}>
          <form onSubmit={handleFormSubmit} className="space-y-6">
            <Tabs defaultValue="outgoing" className="w-full">
              ...
              <TabsContent value="outgoing" className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <Label className="text-base font-semibold">Outgoing Email Engine</Label>
                      <p className="text-sm text-gray-600">Configure settings for sending emails</p>
                    </div>
                    <FormField
                  ...
                  {form.watch("outgoing.enabled") && (
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>Protocol <span className="text-xs text-red-600 font-medium">(Required)</span></FormLabel>
                                <Select 
                                  value={field.value} 
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>Host <span className="text-xs text-red-600 font-medium">(Required)</span></FormLabel>
                                <FormControl>
                          ...
                          <FormField
                            ...
                            render={({ field }) => {
                              ...
                              return (
                                <FormItem>
                                  <FormLabel>Port <span className="text-xs text-red-600 font-medium">(Required)</span></FormLabel>
                                  <FormControl>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>Security <span className="text-xs text-red-600 font-medium">(Required)</span></FormLabel>
                                <Select 
                                  value={field.value} 
                                ...
                                <FormDescription>
                                  {field.value === EmailSecurityEnum.SSL_TLS && "Recommended for most email providers"}
                                  {field.value === EmailSecurityEnum.STARTTLS && "Common for SMTP servers like Gmail"}
                                  {field.value === EmailSecurityEnum.NONE && "⚠️ Not recommended - data will be sent unencrypted"}
                                </FormDescription>
                                <FormMessage />
                              </FormItem>
                            )}
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>Username <span className="text-xs text-red-600 font-medium">(Required)</span></FormLabel>
                                <FormControl>
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>Password <span className="text-xs text-red-600 font-medium">(Required)</span></FormLabel>
                                <FormControl>
                                  <div className="relative">
                                    <Input 
                                      {...field} 
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>From Email <span className="text-xs text-red-600 font-medium">(Required)</span></FormLabel>
                                <FormControl>
                                  <Input {...field} type="email" placeholder="noreply@yourdomain.com" />
                                </FormControl>
                                <FormMessage />
                              </FormItem>
                            )}
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>From Name <span className="text-xs text-red-600 font-medium">(Required)</span></FormLabel>
                                <FormControl>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Reply-To Email <span className="text-xs text-green-600 font-medium">(Optional)</span></FormLabel>
                              <FormControl>
                                <Input {...field} type="email" placeholder="support@yourdomain.com" />
                              </FormControl>
                              <FormDescription>
                      ...
                      {/* Advanced Settings */}
                      <Collapsible open={showAdvancedOutgoing} onOpenChange={setShowAdvancedOutgoing}>
                        <CollapsibleTrigger asChild>
                          <Button variant="ghost" className="flex items-center gap-2 p-0">
                            <Settings2 className="h-4 w-4" />
                            Advanced Settings
                            {showAdvancedOutgoing ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                          </Button>
                        </CollapsibleTrigger>
                        <CollapsibleContent className="space-y-4 pt-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel>Timeout (ms) <span className="text-xs text-green-600 font-medium">(Optional)</span></FormLabel>
                                  <FormControl>
                                    <Input 
                                      type="number" 
                                      {...field} 
                                  ...
                                  <FormDescription>
                                    Connection timeout in milliseconds (1000-300000)
                                  </FormDescription>
                                  <FormMessage />
                                </FormItem>
                              )}
                            ...
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel>Connection Timeout (ms) <span className="text-xs text-green-600 font-medium">(Optional)</span></FormLabel>
                                  <FormControl>
                                    <Input 
                                      type="number" 
                                      {...field} 
                                  ...
                                  <FormDescription>
                                    Socket connection timeout in milliseconds (1000-300000)
                                  </FormDescription>
                                  <FormMessage />
                                </FormItem>
                              )}
                            ...
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel>Max Connections <span className="text-xs text-green-600 font-medium">(Optional)</span></FormLabel>
                                  <FormControl>
                                    <Input 
                                      type="number" 
                                      {...field} 
                            ...
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel>Rate Limit (per second) <span className="text-xs text-green-600 font-medium">(Optional)</span></FormLabel>
                                  <FormControl>
                                    <Input 
                                      type="number" 
                                      {...field} 
              ...
              <TabsContent value="incoming" className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <Label className="text-base font-semibold">Incoming Email Engine</Label>
                      <p className="text-sm text-gray-600">Configure settings for receiving and processing emails</p>
                    </div>
                    <FormField
                      control={form.control}
                      ...
                      <div className="space-y-4">
                                  ...
                                  <FormControl>
                                    <div className="relative">
                                      <Input 
                                        ...
                                        {...field} 
                                        onChange={(e) => field.onChange(Number(e.target.value))}
                                        className={!validation.isStandardPort ? "border-orange-400" : ""}
                                      />
                                      {!validation.isStandardPort && (
                                        <div className="absolute right-2 top-2">
                                          <div className="w-2 h-2 bg-orange-400 rounded-full" title="Non-standard port for this security setting" />
                                        </div>
                                      )}
                                    </div>
                                  </FormControl>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                      ...
                                      <SelectItem key={option.value} value={option.value}>
                                        <div className="flex items-center space-x-2">
                                          <div className={`p-1 rounded-full ${
                                            ...
                                            option.color === 'blue' ? 'bg-blue-500' : 'bg-red-500'
                                          }`}>
                                ...
                                <FormDescription>
                                  {field.value === EmailSecurityEnum.SSL_TLS && "Recommended for IMAP/POP3 connections"}
                                  {field.value === EmailSecurityEnum.STARTTLS && "Upgrade to encryption after connection"}
                                  {field.value === EmailSecurityEnum.NONE && "⚠️ Not recommended - data will be sent unencrypted"}
                                </FormDescription>
                                <FormMessage />
                              </FormItem>
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>Poll Interval (minutes) <span className="text-xs text-red-600 font-medium">(Required)</span></FormLabel>
                                <FormControl>
                                  <Input 
                                    type="number" 
                                    {...field} 
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            render={({ field }) => (
                              <FormItem className="flex items-center justify-between rounded-lg border p-4">
                                <div className="space-y-0.5">
                                  <FormLabel className="text-base">Auto Process Incidents <span className="text-xs text-green-600 font-medium">(Optional)</span></FormLabel>
                                  <FormDescription>
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>Default Priority <span className="text-xs text-red-600 font-medium">(Required)</span></FormLabel>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Auto Assign To <span className="text-xs text-green-600 font-medium">(Optional)</span></FormLabel>
                              <FormControl>
                                <Input {...field} placeholder="User ID or email" />
                              </FormControl>
                              <FormDescription>
                                Automatically assign created incidents to this user
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
            ...
            <Alert>
              ...
              <AlertDescription>
                <strong>📧 Common Email Provider Settings:</strong><br />
                <strong>Gmail:</strong> SMTP: smtp.gmail.com:587 (STARTTLS) or :465 (SSL/TLS) | IMAP: imap.gmail.com:993 (SSL/TLS)<br />
                <strong>Outlook:</strong> SMTP: smtp-mail.outlook.com:587 (STARTTLS) | IMAP: outlook.office365.com:993 (SSL/TLS)<br />
                <strong>Yahoo:</strong> SMTP: smtp.mail.yahoo.com:587 (STARTTLS) | IMAP: imap.mail.yahoo.com:993 (SSL/TLS)<br />
                <br />
                <strong>⚠️ SSL Error Fix:</strong> "wrong version number" usually means port/security mismatch. Use the <strong>Diagnose Config</strong> button below for help.
              </AlertDescription>
            </Alert>
            ...
            <div className="flex items-center gap-3">
              <Button 
                type="submit" 
                disabled={outgoingLoading || incomingLoading || saveEmailMutation.isPending}
                onClick={() => console.log("🚀 ~ Submit button clicked")}
              >
                {(outgoingLoading || incomingLoading || saveEmailMutation.isPending) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              ...
              <Button 
                ...
                onClick={async () => {
                  ...
                  console.log("🚀 ~ Manual submit ~ form errors:", form.formState.errors);
                  const isValid = await form.trigger();
              ...
              <Button 
                ...
                onClick={() => {
                  ...
                  console.log("- Outgoing enabled:", form.watch("outgoing.enabled"));
                  console.log("- Incoming enabled:", form.watch("incoming.enabled"));
                }}
              >
                Debug Info
              </Button>
              ...
              <Button 
                ...
                onClick={() => {
                  ...
                  if (values.outgoing.enabled) {
                    ...
                    diagnostics += `- Standard Config: ${outValidation.isStandardPort ? 'Yes ✅' : 'No ⚠️'}\n`;
                  ...
                  if (values.incoming.enabled) {
                    ...
                    diagnostics += `- Standard Config: ${inValidation.isStandardPort ? 'Yes ✅' : 'No ⚠️'}\n`;
