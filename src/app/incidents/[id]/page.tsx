...
import { ReassignIncidentDialog, ReassignmentData } from '@/components/incidents/ReassignIncidentDialog'
import KnowledgeBaseSection from '@/components/incidents/KnowledgeBaseSection'
...
const createMockIncident = (id: string): IncidentWithDetails => ({
  ...
  description: 'Users are unable to send or receive emails. The server appears to be down since 2 PM. Multiple users from different departments have reported this issue. The issue seems to affect both internal and external email communication.',
...
export default function IncidentDetail() {
  ...
  const handleReassignment = (reassignmentData: ReassignmentData) => {
    ...
    const assignedUser = reassignmentData.assignedToId 
      ? mockUsers.find(u => u.id === reassignmentData.assignedToId)
      : null
    ...
    const updatedIncident: IncidentWithDetails = {
      ...incident,
      assignedToId: reassignmentData.assignedToId || incident.assignedToId,
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            ...
                            <Select 
                              ...
                              onValueChange={(value: IncidentStatus) => setEditData({...editData, status: value})}
                            >
                              <SelectTrigger>
                                <SelectValue />
                              </SelectTrigger>
                    ...
                    {incident.history.map((entry) => (
                      <div key={entry.id} className="text-xs">
                        ...
                        <p className="text-gray-700">
                          ...
                          {entry.oldValue && (
                            <span> from <span className="font-medium">{entry.oldValue}</span></span>
                          )}
                          <span> to <span className="font-medium">{entry.newValue}</span></span>
                        </p>
                      </div>
                    ))}
                  </div>
